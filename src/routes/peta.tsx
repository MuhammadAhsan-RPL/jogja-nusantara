...
export const Route = createFileRoute("/peta")({
  head: () =>
    createSeo({
      ...
      description:
        "Peta interaktif destinasi Yogyakarta: candi, keraton, kota, pantai, alam, dan gua.",
      url: "https://www.jogjadigitalcity.id/peta",
    }),
  component: PetaPage,
});
  ...
  return (
    <article className="mx-auto max-w-[1400px] px-6 py-20 md:px-10 md:py-28">
      <header className="grid gap-10 md:grid-cols-12">
        <div className="md:col-span-8">
          ...
          <h1 className="mt-6 font-display text-5xl leading-[1.02] text-ink md:text-7xl">
            {t("peta.title.a")} <span className="italic text-terracotta">{t("peta.title.em")}</span>{" "}
            {t("peta.title.b")}
          </h1>
        </div>
          ...
          <button
            ...
            className={`border px-4 py-2 font-mono text-[11px] uppercase tracking-[0.2em] transition-colors ${
              filter === c
                ...
                : "border-[var(--color-rule)] text-ink-soft hover:border-ink"
            }`}
          >
      ...
      <div className="mt-8">
        <Suspense
          fallback={
            <div className="flex h-[75vh] items-center justify-center border border-[var(--color-rule)] bg-paper text-sm text-ink-soft">
              {t("peta.loading")}
            </div>
          }
        >
