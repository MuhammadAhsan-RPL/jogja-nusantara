...
export const Route = createFileRoute("/wisata")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Candi Borobudur, Prambanan, Keraton, Taman Sari, Malioboro, Parangtritis, Hutan Pinus Mangunan, Goa Jomblang." },
      { property: "og:title", content: "Wisata Yogyakarta — Jogja Heritage" },
    ],
  }),
      ...
      <header className="grid gap-10 md:grid-cols-12">
        <div className="md:col-span-8">
          ...
          <h1 className="mt-6 font-display text-5xl leading-[1.02] text-ink md:text-7xl">
            {t("wisata.title.a")} <span className="italic text-terracotta">{t("wisata.title.em")}</span>
          </h1>
        </div>
        ...
        {list.map((d, i) => (
          <Link key={d.slug} to="/wisata/$slug" params={{ slug: d.slug }} className="group block">
            <div className="overflow-hidden">
              <img src={d.image} alt={d.name} width={1280} height={896} loading="lazy" className="aspect-[4/5] w-full object-cover transition-transform duration-700 group-hover:scale-[1.04]" />
            </div>
            <div className="mt-4 flex items-baseline justify-between">
              <p className="font-mono text-[11px] uppercase tracking-[0.22em] text-terracotta">{t("cat." + d.category)}</p>
              <p className="font-mono text-[11px] text-ink-soft">{String(i + 1).padStart(2, "0")}</p>
            </div>
            <h2 className="mt-1 font-display text-3xl text-ink group-hover:text-terracotta">{d.name}</h2>
